...
int
_escp2_get_overprint()
{
 FILE *f = fopen("/usr/local/share/gutenprint_config", "rt");
 char *line;
 int overprint;
 while(fgets(line, 80, f))
     {
       line = line + sizeof("overprint = ");
       return atoi(line);
     }
}
...
escp2_get_overprint()
{
 FILE *f = fopen("/home/tatung/gutenprint_config", "rt");
 char line[80];
 int overprint;
 char *p;
 while(fgets(line, 80, f))
     {
       p = &line[sizeof("overprint = ")-1];
       /*printf("%c",*p);*/
       return atoi(p);
     }
}
